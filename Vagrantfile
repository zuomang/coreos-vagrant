  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
      ...
      if File.exist?(CLOUD_CONFIG_PATH)
        if i <= 1
          config.vm.provision :file, :source => "#{$CLOUD_CONFIG_PATH}"+"user-data-master.yaml", :destination => "/tmp/vagrantfile-user-data"
        else
          config.vm.provision :file, :source => "#{$CLOUD_CONFIG_PATH}"+"user-data-slave.yaml", :destination => "/tmp/vagrantfile-user-data"
        end
